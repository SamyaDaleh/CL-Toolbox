...
public class SxCalc {
				...
				for (int l = 1; l <=n; l++) {
					if (l==1) {
						for (PcfgProductionRule rule : cfg.getR()) {
							...
							if (rule.getLhs().equals(nt) && vars.length ==1 && logp < insides.get(getInsideKey(nt,l))) {
	...
	public static Map<String,Double> getOutsides(Map<String,Double> insides, int n, Pcfg pcfg) {
						...
						for (int lc = 1; lc <= nr; lc++) {
							for (PcfgProductionRule rule : pcfg.getR()) {
								...
								if (vars.length == 2 && vars[0].equals(nt)) {
									Double newp = -Math.log(rule.getP());
									newp += outsides.get(getOutsideKey(rule.getLhs(),nl,l+lc,nr-lc));
									newp += insides.get(getInsideKey(vars[1],lc));
									outsides.put(getOutsideKey(nt, nl,l,nr), Math.min(newp, outsides.get(getOutsideKey(nt, nl,l,nr))));
								}
							}
						}
						for (int lc = 1; lc <= nl; lc++) {
							for (PcfgProductionRule rule : pcfg.getR()) {
								...
								if (vars.length == 2 && vars[1].equals(nt)) {
									Double newp = -Math.log(rule.getP());
									newp += outsides.get(getOutsideKey(rule.getLhs(),nl-lc,l+lc,nr));
	...
	private static String getOutsideKey(String nt, int nl, int l, int nr) {
		return "out(" + nt + "," + String.valueOf(nl) + "," + String.valueOf(l) + "," + String.valueOf(nr) + ")";
	}
}
