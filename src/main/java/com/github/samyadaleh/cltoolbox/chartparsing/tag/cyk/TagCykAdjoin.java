...
public class TagCykAdjoin extends AbstractDynamicDecutionRuleTwoAntecedences {
  ...
  public TagCykAdjoin(Tag tag) {
    this.tag = tag;
    this.name = DEDUCTION_RULE_TAG_ADJOIN;
    this.antNeeded = 2;
  }
  ...
  protected void calculateConsequences(String[] itemForm1, String[] itemForm2) {
    ...
    if (!f11.equals("-") && f11.equals(f12) && !f21.equals("-") && f21
        .equals(f22) && tag.isAdjoinable(treeName1, treeName2,
        node2.substring(0, node2.length() - 1)) && node1.equals("⊤") && node2
        .endsWith("⊥")) {
      ...
      if (Arrays.equals(itemForm1, antecedences.get(0).getItemForm())) {
        for (Tree tree1 : antecedences.get(0).getTrees()) {
          for (Tree tree2 : antecedences.get(1).getTrees()) {
            derivatedTrees.add(
                tree2.adjoin(node2.substring(0, node2.length() - 1), tree1));
          }
        }
      } else {
        for (Tree tree1 : antecedences.get(0).getTrees()) {
          for (Tree tree2 : antecedences.get(1).getTrees()) {
            derivatedTrees.add(
                tree1.adjoin(node2.substring(0, node2.length() - 1), tree2));
          }
        }
      }
      String node2Name =
